...
export default function Home() {
  ...
  // Nieuwe state voor granular loading feedback
  ...
  async function processFormSubmission(isPreview, preGeneratedResult = null) {
    ...
    // Note: Mailgun API can be used without SMTP accounts configured
    if (!isPreview && !selectedSmtpId && smtpAccounts.length === 0) {
      // Don't block - Mailgun API will handle sending
      console.log('No SMTP account selected, will use Mailgun API');
    }
  ...
  useEffect(() => {
    ...
    if (loading) {
      ...
      }, 4000); // Elke 4 seconden volgende stap (totale verwachte duur ~20s)
    }
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group full-width">
                      ...
                      {loadingSmtp ? (
                        ...
                        <div className="smtp-mailgun-active">
                          ✅ <strong>Mailgun API actief</strong> - Geen SMTP nodig
                        </div>
                      ) : (
                        <select
...
function FormattedText({ text }) {
  // Always return a consistent structure to avoid React reconciliation errors
  if (!text || typeof text !== 'string') {
    return <div className="section-content"><p>&nbsp;</p></div>;
  }
  ...
  // If all bullets, render as single list
  if (allBullets) {
    return (
      <div className="section-content">
        <ul>
          {lines.map((line, i) => (
            <li key={`bullet-${i}`}>{line.trim().replace(/^[-*•]\s*/, '').trim()}</li>
          ))}
        </ul>
      </div>
    );
  ...
  // Otherwise render as paragraphs (no mixed content to avoid DOM issues)
  return (
    <div className="section-content">
      {lines.map((line, i) => {
        ...
        return <p key={`line-${i}`}>{isBullet ? `• ${content}` : content}</p>;
      })}
    </div>
  );
}
